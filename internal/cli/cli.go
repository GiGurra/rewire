// Package cli is the rewire entrypoint logic, shared by the root
// `main.go` shim (so `go install github.com/GiGurra/rewire@latest`
// works) and the legacy `cmd/rewire/main.go` shim (so the older
// `go install github.com/GiGurra/rewire/cmd/rewire@latest` form
// keeps working). Both shims do nothing but call Run.
//
// rewire has two modes:
//
...
// Run is the rewire entrypoint. It reads os.Args directly and calls
// os.Exit on its own — callers should treat it as terminal.
func Run() {
