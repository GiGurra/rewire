...
Rewire is a Go test mocking tool with three modes that share one toolexec pipeline:
...
2. **Interface mock synthesis (`rewire.NewMock[T]` + toolexec)**: Same toolexec pipeline. Scans test files for `rewire.NewMock[I]` references, locates I's source, and synthesizes a backing struct at compile time. No `go:generate`, no committed files. Handles non-generic AND generic interfaces (Phase 2a — arbitrary type-arg shapes including external-package types and nested generics). Embedded interfaces and types from the interface's declaring package are still rejected with clear errors.
3. **Interface mock CLI (`rewire mock` — legacy `go:generate` style)**: An older standalone CLI that writes a committed `mock_*_test.go` file. Still supported, candidate for deprecation inside rewire once the toolexec NewMock path reaches feature parity (embedded interfaces, declaring-package types).
...
- `internal/mockgen/rewiregen.go` — Toolexec interface mock generator (`GenerateRewireMock`). Parses an interface's source, synthesizes a concrete backing struct, generates per-method dispatch tables, and emits a registration `init()`. Performs AST-level type-parameter substitution for generic interfaces (Phase 2a) and consults a per-instantiation type-arg import map to emit correct imports for type args from packages the interface's declaring file doesn't import.
- `internal/mockgen/mockgen.go` — Legacy CLI mock generator (`GenerateMock`) for the `rewire mock` `go:generate` style. Same general shape as `rewiregen.go` but produces a committed file with function-field mocks. Doesn't yet support generics or embedded interfaces.
...
- **Interface mocks via toolexec (`rewire.NewMock[T]`)**: The current canonical interface-mock path. The scanner detects `rewire.NewMock[I]` in test files, the toolexec generator synthesizes a backing struct for I at compile time, and the struct registers itself with the runtime via `RegisterMockFactory[I]`. Same per-instance dispatch as `rewire.InstanceMethod` for concrete methods. Generic interfaces (Phase 2a) are supported via AST-level type-parameter substitution and per-instantiation factory keys derived from `reflect.TypeFor[I]()`. The legacy `rewire mock` CLI is still supported for users who want committed reviewable mock files.
