...
- **Same dispatch as per-instance method mocks.** Each generated method body consults a per-method `_ByInstance sync.Map` and falls back to zero-value returns. The sync.Map is populated by `rewire.InstanceFunc(t, mock, I.Method, replacement)` — one stubbing verb across concrete per-instance mocks and interface-method mocks.
...
Method mocks set via `rewire.Func` are global — all instances of the type share one package-level mock variable. For per-instance scoping, `rewire.InstanceFunc` emits an additional `Mock_Type_Method_ByInstance sync.Map` and a per-instance dispatch ahead of the global mock in the wrapper body. The extra emission is opt-in at compile time: the scanner only triggers it when it finds `rewire.InstanceFunc` / `rewire.RestoreInstanceFunc` calls referencing the target, so tests that only use `rewire.Func` pay no per-call cost.
...
### API consolidation pass (shipped)

The public API has been through a coherent rename pass: `InstanceMethod → InstanceFunc`, the overloaded `Restore` split into `RestoreFunc` (clear a global function mock) / `RestoreInstance` (clear all per-instance mocks on an instance) / `RestoreInstanceFunc` (clear one (instance, method) pair), and the unused `Replace` escape hatch removed. The `expect` package's `For` / `ForInstance` verbs were kept as-is since they read well as DSL grammar.

### Parallel test safety

Parallel tests that mock the same function race on the shared `Mock_` variable. Options considered: goroutine-local storage (not native in Go), per-test context threading (intrusive), or continuing to document the limitation. We currently ship a pragmatic middle ground: **conflict detection at install time**. Every `rewire.Func` / `rewire.InstanceFunc` call claims ownership of a composite key (`funcName|typeSig`, or `…|receiverPtr` for `InstanceFunc`) in a package-level `ownerRegistry sync.Map`. A second concurrent install by a different `*testing.T` fails with `t.Fatalf` instead of racing silently; reinstalls by the same test are allowed; ownership is released at test end (via `t.Cleanup`) and eagerly by `RestoreFunc` / `RestoreInstance` / `RestoreInstanceFunc`. `InstanceFunc` on *different* receivers — the case users are most likely to hit in `t.Parallel()` code — is handled natively because the ownership key includes the receiver address.

A fully parallel-safe prototype exists on [PR #6](https://github.com/GiGurra/rewire/pull/6): `runtime/pprof`-labels-pointer as goroutine-tree identity, per-function `Mock_Foo_ByGoroutine sync.Map`, children inherit automatically from parent test. Held back because the mechanism leans on an unofficial linkname loophole (reachable today, could close in a future Go release) and the enlarged wrapper is no longer inlined by the compiler (cost ~300 vs the 80-node budget). Full write-up in [`plans/parallel_test_safety_findings.md`](https://github.com/GiGurra/rewire/blob/main/plans/parallel_test_safety_findings.md).
