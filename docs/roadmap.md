...
- **Parallel test safety for the same target.** `t.Parallel()` tests that mock the *same* function with different replacements would race on the shared `Mock_` variable. **Mitigated:** rewire detects the conflict at install time — the second `rewire.Func` / `rewire.InstanceFunc` call fails with `t.Fatalf`, naming the conflicting test, instead of racing silently. `InstanceFunc` on *different* receivers is handled natively (ownership is per-receiver). Parallel tests mocking *different* targets are fine. Full parallel safety via goroutine-inherited pprof labels was prototyped at [PR #6](https://github.com/GiGurra/rewire/pull/6) but held back because it relies on an unofficial linkname loophole and inflates the wrapper past the inliner budget — see [`plans/parallel_test_safety_findings.md`](https://github.com/GiGurra/rewire/blob/main/plans/parallel_test_safety_findings.md).
