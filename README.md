...
- **Parallel mocks on the same target** — two `t.Parallel()` tests that mock the same function with different replacements would race on the package-level mock variable. Rewire is single-test-at-a-time per target, and **detects the conflict at install time**: the second `rewire.Func` / `rewire.InstanceFunc` call fails the test with a clear diagnostic naming the first test. Parallel tests mocking *different* targets (or the same method on *different* receivers via `rewire.InstanceFunc`) are fine. The detector cannot catch the asymmetric case where one parallel test mocks and another parallel test uses the target without calling rewire — see [Limitations](docs/limitations.md#what-detection-cannot-catch-mocker-vs-non-mocker-in-parallel) for details.
